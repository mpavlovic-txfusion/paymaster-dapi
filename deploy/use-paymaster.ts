import { ContractFactory, Contract, Provider, utils, Wallet } from "zksync-web3";
...
function getToken(hre: HardhatRuntimeEnvironment, wallet: Wallet) {
  ...
  return new Contract(TOKEN_ADDRESS, artifact.abi, wallet);
}

function getGreeter(hre: HardhatRuntimeEnvironment, wallet: Wallet) {
  const artifact = hre.artifacts.readArtifactSync("Greeter");
  return new Contract(GREETER_CONTRACT_ADDRESS, artifact.abi, wallet);
}
